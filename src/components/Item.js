...
const Item = ({
  ...
  return (
    <div className="w-full flex border-t border-orange-600 py-4 relative">
      ...
      <div
        className="text-white absolute p-4 right-7 bottom-2 left-[31%]"
      ...
      <div className="w-3/5">
        <div className="flex justify-between gap-4 w-full line-clamp-2">
          <Link
            to={`${path.DETAIL}${formatVietnameseToString(
              title?.replaceAll("/", "")
            )}/${id}`}
            className="text-blue-600  font-medium line-clamp-2"
          >
        ...
        <div className="my-2 flex md:flex-row flex-col md:items-center gap-2">
        ...
        <p className="text-gray-500 hidden md:flex w-full h-[50px] text-ellipsis overflow-hidden">
          {description}
        </p>
        <div className="hidden flex-col md:flex-row md:items-center my-5 justify-between">
