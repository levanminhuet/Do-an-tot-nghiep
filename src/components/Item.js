...
const Item = ({
  ...
  return (
    <div className="flex border-t border-orange-600 py-4 ml-5 mr-5">
      <Link
        to={`/chitiet/${formatVietNam(title?.replaceAll("/", ""))}/${id}`}
        className="w-1/4 flex flex-wrap gap-[2px] items-center relative cursor-pointer"
      >
        {images.length > 0 &&
          images
      ...
      <div className="w-3/4">
        <div className="flex justify-between gap-4 w-full">
          <Link
            to={`/chi-tiet/${formatVietNam(title?.replaceAll("/", ""))}/${id}`}
