  ...
  return (
    <div className="relative h-full bg-white p-4">
      <div className="flex items-center justify-between gap-8 border-b">
        <h3 className="font-bold text-blue-400 text-[30px] pb-4 ">
            ...
            {reports?.rows?.map((item, index) => (
              <tr key={item.id}>
                  ...
                  {item?.reportPost ? (
                    <Link
                      to={`${path.DETAIL}${formatVietnameseToString(
                        item?.reportPost?.title?.replaceAll("/", "")
                      )}/${item?.reportPost?.id}`}
                      className="text-blue-600 text-l font-semibold"
                    >
                ...
                >
                  {item?.status !== "Pending" && (
                    <span
                      className="flex justify-items-center justify-center cursor-pointer text-blue-500 hover:underline"
